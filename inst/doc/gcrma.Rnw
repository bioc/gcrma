...
\begin{document}
\title{Textual description of gcrma}
...
You will need to intall the {\tt affy} package. 
and the a package containing probe sequence data (such as hgu95av2probe).
...
\begin{Sinput}
R> library(gcrma) ##load the affy package
\end{Sinput}
